...
btnEntrar.addEventListener('click', e => {
    ...
    fetch(`${window.location.origin}/login`, {
        method: 'POST', 
        headers: {'Content-Type': 'application/json'},  // ta informando que o conteudo é pelo JSON
        body: JSON.stringify(usuario) // Convertendo objeto para String
      })
        .then(response => response.status)  // Se a resposta seja bem-sucedida, será realizado o login
            .then(statusCode => {
            if (statusCode == '200') {
                window.location.href = './pages/library.html';
            }else if ((statusCode == "401") || (statusCode == "500")) { // Caso não, vai informar que o usuario não foi encontrado
